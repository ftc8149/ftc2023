...
@Autonomous(name="Odometry Based Movement")
public class FinalOdometry extends LinearOpMode {
    ...
    private void goToPosition(double changeX, double changeY, double targetAngle, double speed, double angularSpeed, double positionTolerance, double angleTolerance) {
        ...
        double rotatedX = xPos * Math.cos(angle) + yPos * Math.sin(angle);
        double rotatedY = -xPos * Math.sin(angle) + yPos * Math.cos(angle);
        xPos = rotatedX;
        yPos = rotatedY;
        ...
        while (opModeIsActive() && (distance > positionTolerance)) {
            ...
            angle = ((((getMotorPosition("FL") - lastFL) / TICKS_PER_INCH) - ((getMotorPosition("FR") - lastFR) / TICKS_PER_INCH)) / (2 * LR_RADIUS)) - lastAngle;
            xPos = -1 * ((getMotorPosition("RR") - (CENTER_TO_AUX * angle))) - lastXPos; // - horizontalOffset after 4.896646*angle
            rotatedX = xPos * Math.cos(angle) + yPos * Math.sin(angle);
            rotatedY = -xPos * Math.sin(angle) + yPos * Math.cos(angle);
            xPos = rotatedX;
            yPos = rotatedY;
            ...
            double xSpeed = speed * Math.cos(angleToTarget - angle) * distanceRatio;
            double ySpeed = speed * Math.sin(angleToTarget - angle) / distanceRatio;
